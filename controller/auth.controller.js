...
//cloudinary pfp uplaod to datebase
const {
  uploadSingle,
  uploadToCloudinary,
...
exports.register = asyncHandler(async function (req, res) {
  ...
  //cloudinary pfp uplaod to datebase #2
...
exports.login = asyncHandler(async function (req, res, next) {
  ...
  // // 7-if the active :false => true
  // const userCredential = await UserCredential.findOne({ user: req.params.id });

  // if (userCredential && !userCredential.active) {
  ...
  // 8- send access token as response
  const { statusCode, body } = success({
